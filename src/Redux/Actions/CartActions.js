...
export const listCart = () => async (dispatch) => {
  try {
    ...
    const { data } = await client.get("/carts/getAll", config);
    if (data.productItem) {
...
export const increaseCart =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await client.post(
        `/carts/increproductInCart/${cartID}`,
        dataProduct
      );
...
export const decreaseCart =
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await client.post(
        `/carts/decreproductInCart/${cartID}`,
        dataProduct
      );
  ...
  async (dispatch) => {
    try {
      ...
      const { data } = await client.delete(
        `/carts/delproductInCart/${cartID}/${productID}`
      );
